...
def train():
  ...
  def weight_variable(shape):
    ...
    initial = tf.random_normal(shape) # NEED TO REPRODUCE THE BUG
  ...
  hidden1 = tf.nn.dropout(hidden1, keep_prob) # NEED TO REPRODUCE THE BUG
  hidden2 = nn_layer(hidden1, 500, 500, 'layer2')
  # hidden2 = tf.nn.dropout(hidden2, keep_prob)

  # hidden2 = hidden1 # if ignore second layer everything is ok
  # Do not apply softmax activation yet, see below.
  y = nn_layer(hidden2, 500, 10, 'layer_out', act=tf.identity)
  ...
  #   # grads = tf.Print(grads, [grads], summarize=10)
  ...
  for i in range(FLAGS.max_steps):
    if i % 100 == 0:  # Record summaries and test-set accuracy
      ...
      if i % 20 == 0:  # Record execution stats
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--log_dir',
      type=str,
      default='/tmp/tensorflow/mnist/logs/mnist_with_summaries_bug',
